function chose_music () {  
    var minrandom = 0;
    var maxrandom = 399; //399
    var random = Math.floor( Math.random() * ((maxrandom + 1) - minrandom) + minrandom );
    var items = [
        'ロキ', 
        '表裏一体', 
        'ナンセンス文学', 
        '太陽系デスコ', 
        'エイリアンエイリアン', 
        '脱法ロック', 
        '気まぐれメルシィ', 
        'against the world', 
        '崩壊歌姫-disruptive diva-', 
        '未完成タイムリミッター', 
        'シエレトワール', 
        'アユミ☆マジカルショータイム', 
        'IMAGINARY LIKE THE JUSTICE', 
        '林檎花火とソーダの海', 
        'アウルソング', 
        '妄想犯', 
        'アダルトチルドレン', 
        'ガランド', 
        '青', 
        '啼くowl', 
        'あの夏が飽和する。', 
        'プラシーボ', 
        'clock lock works', 
        'モノノケミステリヰ', 
        'まっさかさまー', 
        'エヴィサレーション', 
        'アイ', 
        '好きって言って', 
        'ムゲンモラトリアム', 
        'バイオレンストリンガー', 
        'センセーションはおわらない！', 
        'きみはだいじょうぶ', 
        '365日綴り続けた恋文のように', 
        '灯調', 
        'レム', 
        'CatDog', 
        'レードニア', 
        'スーサイドリピーター', 
        '君が飛び降りるのなら', 
        'ハイスペックニート', 
        'アンノウン・マザーグース', 
        '初音ミクの消失-DEAD END-', 
        '君にとって', 
        'ミルククラウン・オン・ソネーチカ', 
        'Knife On The Dancefloor.', 
        'アリストラスト', 
        '何もない様な', 
        'レテノール', 
        'ラストライトラブソング', 
        'あわよくばきみの眷属になりたいな', 
        'フォッサマグナ', 
        'ある夏のシュプレヒコール', 
        '命ばっかり', 
        'Tsun Tsun', 
        'LUVORATORRRRRY!', 
        'クレイジー・ビート', 
        'い～やい～やい～や', 
        'Makes You a Fighter', 
        'ピュライド', 
        'RE-OVERDOSE', 
        'ミクがネギを背負ってやって来る', 
        'リバースユニバース', 
        'エクスチェンジ × ホラーナイト', 
        'のぼれ！すすめ！高い塔', 
        'だれかの心臓になれたなら', 
        'M.S.S.Planet', 
        '贅沢病', 
        'Toy', 
        '1925', 
        'Harvest', 
        '戯言の雑音', 
        'ロストアンブレラ', 
        'ハナイロ☆シャイガール', 
        '絶対的少年値', 
        '神のまにまに', 
        'プロトディスコ', 
        '乙女解剖', 
        'あのこどこのこ', 
        'LaLaL危', 
        'ぜろ', 
        '愛言葉Ⅲ', 
        '延命治療', 
        'letter song', 
        '祝福のメシアとアイの塔', 
        'モーメント', 
        'ニア', 
        'ノラボク', 
        'Ten Thousand Stars', 
        'リトルパレード', 
        '頓痴気乱痴気珍吞紀行', 
        'さよなら微炭酸がぁる', 
        'ポジティブ☆ダンスタイム', 
        'スラッガー', 
        '独白ラビリンス', 
        'マトリョシカ', 
        '初音ミクの激唱(LONG VERSION)', 
        'ハグ', 
        '劇場愛歌', 
        'サンクチュアリ', 
        '藍色ベリル', 
        '透明エレジー', 
        'エゴロック', 
        'Q', 
        'アスター', 
        '帝国少女', 
        'さんきゅ', 
        'タピオの記憶', 
        '心片整理', 
        '8.32', 
        'アッカンベーダ', 
        '妄想感傷代償連盟', 
        'Blessing', 
        'インタビュア', 
        '1/6-out of the gravity-', 
        'ECHO', 
        'ヒビカセ', 
        '古に封印されし究極黒魔法(物理)', 
        'オノマトペテン師', 
        'エキソダス', 
        'フィクサー', 
        'DAYBREAK FRONTLINE', 
        'Ever', 
        'Emile', 
        '僕みたいな君 君みたいな僕', 
        '水死体にもどらないで', 
        '青に溶ける', 
        'セルリアンブルー錯視症', 
        '愛影(リマスター版)', 
        '内臓ありますか ', 
        '赤黒い言葉', 
        '悪ノ召使 ', 
        '小さな恋のうた(Synth Rock Cover)', 
        'ボイジャーズ', 
        'スキスキ絶頂症', 
        'さよならワンダーノイズ', 
        '空中の花束', 
        'ウミユリ海底譚', 
        'ビイドロ', 
        '失敗作少女', 
        'セブンスコード', 
        '天樂', 
        'こどものしくみ', 
        '怪盗Fの台本(シナリオ)～消えたダイヤの謎～', 
        '臨海ダイバー', 
        '夜咄ディセイブ', 
        'ライムライト', 
        'ダンスワナビーダンス', 
        '永眠童話', 
        '蝶と花と蜘蛛', 
        '光よ', 
        '言葉で戦うということ', 
        '大切な人たちへ', 
        'Draw', 
        '細菌汚染-Bacterial Contamination-', 
        '悪魔の踊り方', 
        '名も無き囚人', 
        'ゴシップ・ミュージック・パーリナイ', 
        'テオ', 
        'スロウダウナー', 
        '飛行少女と僕', 
        'シンクロナイズドローム', 
        '平成のこと', 
        'ラストダンス', 
        'バビロン', 
        '拝啓ドッペルゲンガー', 
        'アヤカシ', 
        '存在のアンサー', 
        '恐怖！不透明人間', 
        '性格悪くてすみません。', 
        '素直じゃなくてごめんなさい。', 
        'グローリーグローリークライム', 
        '名前のない怪物(カバー)', 
        '全く身にならないソング', 
        'ライカ', 
        '失想ワアド', 
        'インターネッツ・ディスコ', 
        '完全犯罪ラブレター', 
        'トオトロジイダウトフル', 
        'ドクター＝ファンクビート', 
        'スクランブル交際', 
        '怪盗黒猫シャノアール', 
        '雲散霧消', 
        'Fireworks', 
        'ゆめゆめ', 
        'Sweet Rain', 
        'スマートを模索する', 
        '融解', 
        '猫の食卓', 
        '嗤うマネキン', 
        'never', 
        '水音とカーテン', 
        'アスノヨゾラ哨戒班', 
        'エンダーエンバーミング', 
        '廃園遊戯', 
        'ODDS&ENDS(カバー合唱)', 
        '心から透明', 
        'ジャガーノート', 
        '朝焼けとワンダーランド', 
        '星屑オーケストラ', 
        '地球最後の告白を', 
        'ビターチョコデコレーション', 
        'ダーリン', 
        '再演', 
        'クーラーガール', 
        'エゴイスト', 
        'Alice in冷凍庫', 
        'DE-Pression', 
        'ヒトサマアレルギー', 
        'しんでしまうとはなさけない！', 
        'スーサイドパレヱド', 
        '色喰いの鬼', 
        '笑門来福', 
        '勝算◎最前線', 
        'はやくそれになりたい！', 
        'グリーンスムージー', 
        'パンダヒーロー', 
        'ストラトステラ', 
        'マチガイサガシ', 
        'メリーバッドエンド', 
        'アンベシル滑落奇譚', 
        '茜音色の蝉時雨', 
        'それでも生きてみた。', 
        '天ノ弱', 
        '在来ヒーローズ', 
        '迷々', 
        'イカサマライフゲイム', 
        'ゴーストルール', 
        '月旅', 
        'longing', 
        '一夜落花', 
        'Nice to meet you', 
        'Ievan Polkka(カバー)', 
        'ストロボラスト', 
        'メーベル', 
        'トキヲ・ファンカ', 
        '金星のダンス', 
        '全速力協奏曲', 
        'METEOR', 
        'メルト', 
        '春に一番近い街', 
        'バイタルサイン', 
        'ブスに巨乳って意味あんのかよ', 
        'キャラバン', 
        'ドリームレス・ドリームス', 
        '晴天前夜', 
        'ディダダダダ', 
        'ブッキッシュタウンヒッピー', 
        '鋼鉄ノ鳥', 
        'おどりゃんせ', 
        'ハッピーヒロインスナイパー', 
        'ジグソーパズル', 
        'ブランコ乗りのサン=テグジュペリ', 
        '快晴', 
        'モーニング・グローリー', 
        'Calc.', 
        'キャットフード', 
        'テスラは夢の中', 
        'トゥイー・ボックスの人形劇場', 
        'ぼうけんのしょがきえました！', 
        'ブリキノダンス', 
        '浮遊月光街', 
        'シロクロリバーシ', 
        'バカをやるなら', 
        'ひよこと天秤', 
        'ツギハギエデン', 
        'ヲズワルド', 
        'Lust Blue', 
        '幾望の月', 
        '悪ノ娘', 
        'Lambency', 
        'とても痛い痛がりたい', 
        'サイバーサンダーサイダー', 
        'm/es', 
        '閻魔さまのいうとおり', 
        'nee', 
        '*ハロー、プラネット。', 
        'サイハテ', 
        '教育', 
        '邪魔', 
        'バレリーコ', 
        'ナッシングバット・スカイ', 
        'ビビッドヒーロー', 
        'イコン', 
        '捨て子のステラ', 
        'エンヴィキャットウォーク', 
        '再教育', 
        'ハローアイザック', 
        '愛に奇術師', 
        '∞まわる∞', 
        'レアノ', 
        'ドラマツルギー', 
        'テールランプと金魚鉢', 
        'ラットが死んだnew words w', 
        'レコリクションエンドロウル', 
        'センチメンタルキネマ', 
        'URUSaaA愛', 
        '廃墟の国のアリス', 
        '紗痲', 
        'インヤンカンケイ', 
        'ラブソングを殺さないで', 
        'マインドブランド', 
        '始発とカフカ', 
        '生命線', 
        'アストロノーツ', 
        'それがあなたの幸せとしても', 
        '嘘未来', 
        '少女レイ', 
        '幻奏歌', 
        'Re-quest', 
        'レッド・パージ！！！', 
        'ニビイロドロウレ', 
        '愛して愛して愛して', 
        '心傷ラストナイト', 
        '3月5日の光', 
        'ひとはおばけになる', 
        'カトラリー', 
        'ドライドライフラワー', 
        'レキシコン', 
        '花落とし', 
        '命に嫌われている。', 
        'グリーンライツ・セレナーデ', 
        'ディザーチューン', 
        'チルドレンレコード', 
        'ロスタイムメモリー', 
        '洗脳', 
        'Just Be Friends', 
        '格子の心臓', 
        'オカワリヲドーゾ', 
        'カクメイ', 
        '神曲', 
        '幼日', 
        'About me', 
        'a with', 
        '-ERROR', 
        '例えば、今此処に置かれた花に', 
        '背景、夏に溺れる', 
        '夜明けと蛍', 
        '清福', 
        '贖罪', 
        '声', 
        '雨き声残響', 
        'Love*3', 
        'Reboot', 
        'FREELY TOMORROW', 
        'Little Scarlet Bad Girl', 
        'アイボリー', 
        'ウタカタ永焔鳥', 
        'ロボトニー', 
        'ハロー人生 さよならメーデー', 
        '各駅停車', 
        'プレイ', 
        '最初の一歩から', 
        'name of us', 
        '夜行性ハイズ', 
        'っょぃゃっょゎぃゃっ', 
        '幸せ者', 
        'シャルル', 
        'おじゃま虫', 
        '或る愛の日', 
        'メルクリウス', 
        'CITRUS', 
        '縫口', 
        'ロンリーダンス', 
        'ロストワンの号哭', 
        'アルカリレットウセイ', 
        'すろぉもぉしょん', 
        'メグメグ☆ファイアーエンドレスナイト', 
        'ユカユカ☆ヘヴンリーナイト', 
        'マダラカルト', 
        '帰巣本能-少年by the mile-', 
        'ちがう!!!', 
        'AR-Augmented Reality-', 
        'スーパーチャージマッハー', 
        'サマータイムレコード', 
        '六兆年と一夜物語', 
        '炭酸ラビッシュ', 
        'ロミオとシンデレラ', 
        'なんてね。', 
        'グラーヴェ', 
        '自由に捕われる。', 
        'オレンジ', 
        'celluloid', 
        'Life', 
        '誰かのヒーローになりたかった', 
        '忘れたいことばっかだ', 
        '砂の惑星', 
        '天使だと思っていたのに', 
        'ウレイ', 
        'カゲロウデイズ', 
        'アディショナルメモリー', 
        'パシフィックバター', 
        '雨とペトラ(Remix)', 
        '雨とペトラ', 
        '東京テディベア', 
        'シンナーガール', 
        'とても素敵な六月でした', 
        'セカイ再信仰特区', 
        'イヤホンと蝉時雨', 
        'Christmas Morgue', 
        '林檎売りの泡沫少女', 
    ];
    document.getElementById("show_text").innerHTML = `選ばれたのは...<span class="span-big"><a href="https://www.youtube.com/results?search_query=` +  items[random] + `%20official" class="span-big-url">` + items[random] + `</a></span>です!`;
    gazou(imgs[random]);
};

function gazou(url) {
    if (url != undefined) {
        document.getElementById("iframe-yt").innerHTML = `<iframe src="https://www.youtube.com/embed/` + url `" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`;
    }
};